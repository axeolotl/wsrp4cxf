
package oasis.names.tc.wsrp.v1.intf;

import javax.xml.ws.WebFault;


/**
 * This class was generated by Apache CXF 2.1.1
 * Wed Aug 13 15:22:22 CEST 2008
 * Generated source version: 2.1.1
 * 
 */

@WebFault(name = "UnsupportedWindowState", targetNamespace = "urn:oasis:names:tc:wsrp:v1:types")

public class UnsupportedWindowState extends Exception {
    public static final long serialVersionUID = 20080813152222L;
    
    private oasis.names.tc.wsrp.v1.types.UnsupportedWindowStateFault unsupportedWindowState;

    public UnsupportedWindowState() {
        super();
    }
    
    public UnsupportedWindowState(String message) {
        super(message);
    }
    
    public UnsupportedWindowState(String message, Throwable cause) {
        super(message, cause);
    }

    public UnsupportedWindowState(String message, oasis.names.tc.wsrp.v1.types.UnsupportedWindowStateFault unsupportedWindowState) {
        super(message);
        this.unsupportedWindowState = unsupportedWindowState;
    }

    public UnsupportedWindowState(String message, oasis.names.tc.wsrp.v1.types.UnsupportedWindowStateFault unsupportedWindowState, Throwable cause) {
        super(message, cause);
        this.unsupportedWindowState = unsupportedWindowState;
    }

    public oasis.names.tc.wsrp.v1.types.UnsupportedWindowStateFault getFaultInfo() {
        return this.unsupportedWindowState;
    }
}
